...
export async function analyzeImageForBeauty(base64Image: string): Promise<BeautyAnalysis> {
  ...
  if (!API_KEY) {
    ...
    throw new Error('کلید API برای این برنامه تنظیم نشده است. لطفا با مدیر برنامه تماس بگیرید.');
  }
  ...
  try {
    ...
    if (errorMessage.toLowerCase().includes('fetch failed') || errorMessage.toLowerCase().includes('networkerror')) {
        throw new Error("خطای شبکه. ممکن است دسترسی به سرویس هوش مصنوعی از منطقه شما مسدود باشد. لطفا اتصال اینترنت خود را بررسی کنید.");
    }
